...
def main(logdir="/input-logfiles", outdir="./parsed"):
    ...
    p1 = Process(target=parse_access_worker,
                 args=(access_log, tmp_access_entry, tmp_access_detail))
    p2 = Process(target=parse_dataxceiver_worker,
                 args=(datax_log, tmp_datax_entry))
    p3 = Process(target=parse_namesystem_worker,
    ...
    merge_action_types(
        [tmp_access_actions, tmp_datax_actions, tmp_namesys_actions], outdir
    )
